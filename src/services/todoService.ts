...
export const todoService = {
  ...
  async getTodos(params: PaginationParams = {}): Promise<APIResponse<Todo[]>> {
    ...
    const response = await api.get<APIResponse<Todo[]>>(`/todos?skip=${skip}&limit=${limit}`);
  ...
  // Update todo
  async updateTodo(todoId: string, updates: UpdateTodoData): Promise<APIResponse<Todo>> {
    const response = await api.put<APIResponse<Todo>>(`/todos/${todoId}`, updates);
    return response.data;
  },
  ...
  async toggleTodoCompletion(todoId: string): Promise<APIResponse<Todo>> {
    const response = await api.patch<APIResponse<Todo>>(`/todos/${todoId}/toggle`);
    return response.data;
  },
}; 