...
import type { Todo, TodoState, TodoStats, CreateTodoData, UpdateTodoData } from '../types/todo';
...
export function TodoProvider({ children }: { children: ReactNode }) {
  ...
  const fetchTodos = async (page: number = 1) => {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to fetch todos';
  ...
  const addTodo = async (todoData: CreateTodoData) => {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to create todo';
      dispatch({ type: 'SET_ERROR', payload: errorMessage });
      throw error;
    }
  };
  ...
  const toggleTodo = async (id: string) => {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to toggle todo';
      dispatch({ type: 'SET_ERROR', payload: errorMessage });
      throw error;
    }
  };
  ...
  const getStats = (): TodoStats => {
    ...
    const onTime = completedTodos.filter(todo => 
      format(parseISO(todo.created_at), 'yyyy-MM-dd') === format(parseISO(todo.updated_at), 'yyyy-MM-dd')
    ).length;
